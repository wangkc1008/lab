...
new_data = data[corr_data[(corr_data > 0) | (corr_data < -0)].index]
print('new_data', new_data.shape)
...
hidden1_2 = tf.layers.dense(x, 600, activation=tf.nn.relu)
# hidden1_2 = tf.layers.dropout(hidden1_2, dropout_rate)
hidden2_2 = tf.layers.dense(hidden1_2, 400, activation=tf.nn.relu)
